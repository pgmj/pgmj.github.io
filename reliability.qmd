...
This post has two purposes. The primary one is to compare some reliability metrics, since a new paper [@bignardi_general_2025] inspired some ideas. The main idea I had was to use plausible values instead of fully Bayesian model posterior draws to estimate the "relative measurement uncertainty" (RMU) metric proposed by Bignardi and colleagues [-@bignardi_general_2025]. The secondary purpose is to show how to fit Rasch models, dichotomous and partial credit models, using Bayesian models with `brms`, and make some comparisons of person and item parameters relative CML/WLE estimation. There is already an excellent paper on Bayesian Item Response Modeling [@burkner_bayesian_2021], but I thought a blog post like this might still be a useful addition to learning materials available. And also an opportunity for myself to learn.
...
We won't check model fit, just assuming things work here, to see how the reliability metrics compare and how you specify Rasch models using `brms`. Weakly informative priors are used for item parameters, borrowing from Bürkner [-@burkner_bayesian_2021]. We use four parallel chains, each with 2000 iterations of which half are warmup, resulting in 4000 draws from the posterior distribution. For comparison, we also fit the same model with 8000 draws (not shown below).
...
Also did some timing tests, comparing `TAM` and `mirt` packages, where the latter has options in how to estimate thetas (latent scores) used for generating the plausible values. Since WLE is generally considered to have the lowest bias [@kreiner_specific_2025], this is the default choice in the `RIreliability()` function.

```{r}
#| code-fold: true
# defining a separate function for RIreliability in case I make changes later on in the easyRasch package
...
WLE and EAP with `mirt` are equal in this run, both at 8.1s. `TAM` takes about 31s.

### Conditional reliability

This function and its code is borrowed from [@mcneish_reliability_2025]. Reporting alpha and omega with 95% confidence intervals.
...
This is actually a bit tricky. Since RMU uses a random pairing of draws, there is variation in the estimates, both point values and HDCIs. Let's see how much it varies.
...
rel %>%
  ...
  labs(title = "RMU random variation",
       subtitle = "Using a `brms` model with 4000 posterior draws, estimating RMU 1000 times",
...
rel8 %>%
  ...
  stat_slabinterval() +
  labs(title = "RMU random variation",
       subtitle = "Using a `brms` model with 8000 posterior draws, estimating RMU 1000 times",
       x = "Relative Measurement Uncertainty",
       y = NULL) +
  theme_minimal() +
  theme(legend.position = "none")
...
Pretty much the same with 8000 draws as with 4000.

How about just estimating RMU 50 times, does it converge on the same mean values for point and CI estimates, compared to estimating RMU 1000 times?
...
Point estimate varies from .761 to .764, with lower/upper CI ranging from .694-.698 and .825 to .827. Two decimal precision may not be reached always, but I think this is close enough. Estimates seem unlikely to vary more than 0.01.
...
To run a partial credit model (PCM) with `brms`, we need `family = "acat"`, which defaults to use logit link. 
