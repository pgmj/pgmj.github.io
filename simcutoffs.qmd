...
parameters. This uses the 1st and 99th percentile value from *all* items as cutoff
for all items. This is not ideal, but was the easiest to implement. There will be "custom" cutoffs for every item in the near future. `RIgetfitTable()` will show the simulation based cutoff values from each item, see below.

Unfortunately, `RIitemfitRM()` has not yet been updated to make use of simulation based cutoffs.

```{r}
RIitemfitPCM(pcmdat2, simcut = TRUE, gf = itemfit_sim)
```

Values crossing the cutoff thresholds are automatically colorized red. The cutoff values used in the above table can be shown with `RIgetfitLoHi()`.
...
the function `RIgetfitTable()` is used with the output object from `RIgetfit()` as input. The table defaults to show 1st and 99th percentile values as lower/upper values.
...
As you can see, there is some variation in the different fit metrics across the items, which highlights the need for indicating this in the `RIitemfitPCM()` table with separate cutoff values for all items/metrics. Colors in the plot are for 1st/99th percentile, dots are median values.
