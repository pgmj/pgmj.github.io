...
All items seem to fit the Rasch model quite well. Values crossing the cutoff thresholds would be colorized red.

If you want to see a table with the individual item fit upper/lower cutoffs,
the function `RIgetfitTable()` should be used with the output object from `RIgetfit()` as input.
...
As you can see, there is variation in the different fit metrics across the items, which strengthens the need for indicating this in the `RIitemfitPCM()` table.
