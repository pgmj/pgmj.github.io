...
Based on the simulations I have made so far, it seems that 1000 iterations may be sufficient for a reasonably accurate 99% percentile value (`q99`) value. The max value will increase with the number of iterations, and it seems to be a bit spurious, so I would suggest to use the 99% percentile value as the cutoff. For more detailed information on the topic I refer to Christensen et al. [-@christensen2017].
...
for all items. This is not ideal, but was the easiest to implement. There will be "custom" cutoffs for every item in the near future. Also, `RIitemfitRM()` has not yet been updated to make use of simulation based cutoffs.
...
It also seems most reasonable to use the 99th (upper/lower) percentile for item fit metrics. I will be implementing this change in a future update to the `RISEkbmRasch` package.
