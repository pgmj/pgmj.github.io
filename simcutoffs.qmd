...
While we are mostly interested in the 99th quartile from the simulation, we will save the output of the simulation into the object `res_cor`.
...
Based on the simulations I have made so far, it seems that 500 iterations is sufficient for a reasonably accurate q99 value. The max value will increase with the number of iterations, and it seems to be a bit spurious, so I would suggest to use the 99% value as the cutoff. For more detailed information on the topic I refer to Christensen et al. [-@christensen2017].
