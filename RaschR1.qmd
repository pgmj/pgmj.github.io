...
### Floor/ceiling effects on raw data
...
items is `r floor_eff`%, while the proportion who respond in the highest
category on all items is `r ceiling_eff`%.
...
pi.locations <- data.frame(matrix(ncol = 3, nrow = 2))
#provide column names
...
plotPWmap(df.erm, pmap = T, imap = F)
          #personCI = list(gamma=0.95,col="red",lty="dotted"))
...
**`r round(nCeilingPfit/length(pthetas)*100,1)`%** have person infit t above the cutoff 2.0 and **`r round(nFloorPfit/length(pthetas)*100,1)`%** are below -2.0.
...
item.fit.table %>% 
  ...
  geom_vline(xintercept = -2, color = "chartreuse3", linetype = 2) +
  geom_vline(xintercept = 2, color = "chartreuse3", linetype = 2) + 
  geom_text(hjust=1.5)
...
### First contrast loadings vs item location
...
pcaloadings %>% 
  ...
  ylab("Loadings") +
  xlab("Item location") +
  geom_vline(xintercept = 0, color = "chartreuse3", linetype = 2) +
  geom_hline(yintercept = 0, color = "chartreuse3", linetype = 2) + 
