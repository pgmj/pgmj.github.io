...
The `iarm` package provides a function to assess DIF by partial gamma [@bjorner_differential_1998]. 
...
- Gamma outside the interval 20.31–10.31, **and**
- significantly outside the interval 20.21–10.21

```{r}
partgam_DIF(as.data.frame(df), dif.sex, p.adj = "BH")
```
...
RIitemfit(df, simcut = simfit3)
```
#### Without misfitting respondents
```{r}
