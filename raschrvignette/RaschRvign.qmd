...
Below you will find a custom theme function, `theme_rise()`, which might be easier to use if you are not familiar with `theme()`.
...
```{r}
# first remove the Person Scores variable
df2$PersonScores <- NULL

RIitemHierarchy(df2) +
  theme_minimal() + # first apply the minimal theme
  theme_rise(fontfamily = "Palatino") # then apply theme_rise, which simplifies making changes to all plot elements
```

If you only want to change the font and not the background, remove the line of code with `theme_minimal()`.
