...
As of package version 0.1.16 there are four new functions for analyzing item location DIF. These are all making use of the function `LRtest()` from the `eRm` package. A limitation of the current function in the `RISEkbmRasch` package is that only two groups can be used in the DIF variable. A future release will enable use of any number of groups.
