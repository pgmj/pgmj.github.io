...
Since the distribution of item fit statistics are not known, we need to make simulations to get appropriate cutoff threshold values. Notably, the new (since version 0.2.2, dated 2024-08-19) `RIitemfit()` function uses conditional outfit/infit, which is both robust to different sample sizes and makes ZSTD unnecessary [@muller_item_2020]. `RIitemfit()` can also use the simulation based cutoff values and use them for conditional highlighting of misfitting items.

The simulation will take quite a bit of time to run if you have complex data/many participants, and/or choose to use many iterations

```{r}
simfit <- RIgetfit(df, iterations = 1000, cpu = 8) # save output into object `simfit` (arbitrary name)
RIitemfit(df, simcut = simfit)
```

`RIitemfit()` works with both dichotomous and polytomous data.
...
```{r}
simfit2 <- RIgetfit(df, iterations = 1000, cpu = 8) # save output into object `simfit` (arbitrary name)
...
```{r}
res_cor2 <- RIgetResidCor(df, iterations = 1000, cpu = 8)
RIresidcorr(df, cutoff = res_cor2$p99)
```
...
https://pgmj.github.io/simcutoffs.html
