...
Since we have a sample size over 500, ZSTD item fit values would be inflated [@hagell2016;@muller_item_2020] if we used the whole sample. To better estimate accurate ZSTD values, the `RIitemfitPCM()` function allows for multiple subsampling. While it has been recommended to use a sample size between 250 and 500 [@hagell2016], it is better to keep the sample size at 200-250 when possible  [@muller_item_2020]. If your data has few responses in some response categories for some items, you may need to try larger number of participants.
...
out_infit(r)
# for more correct p-values using bootstrapping, use `boot_fit(r, B = 100)`
```

For a more clean layout and conditional highlighting outside of 0.7-1.3:

```{r}
cfit <- out_infit(r)
data.frame(Outfit = cfit$Outfit,
           Infit = cfit$Infit) %>% 
  round(3) %>% 
  rownames_to_column("Item") %>% 
  mutate(Outfit = cell_spec(Outfit, 
                            color = ifelse(Outfit < 0.7, "red", 
                                           ifelse(Outfit > 1.3, "red", "black"))),
         Infit = cell_spec(Infit, color = ifelse(Infit < 
                                                   0.7, "red", ifelse(Infit > 1.3, 
                                                                      "red", "black")))) %>% 
  kbl_rise(fontfamily = "Lato", tbl_width = 35)
```
...
Item 15 shows slightly low item fit.
