...
Based on an old rule-of-thumb, the first eigenvalue should be below 1.5 to support unidimensionality [@smith_detecting_2002]. However, as with many other metrics this is dependent on sample size and test length [@chou_checking_2010] and needs to be used together with checking item fit (or item-restscore) and residual correlations (local dependence). I personally find this metric less useful and prefer to use the Conditional Likelihood Ratio Test as a global test of model fit. The PCA eigenvalues are only included here for those coming from Winsteps who might be looking for it. Speaking of Winsteps, the "explained variance" will not be comparable to Winsteps corresponding metric, since this one only shows the results from the analysis of residuals and not the explained variance of the Rasch model itself.

### Conditional LRT

The Conditional Likelihood Ratio Test [LRT, @andersen_goodness_1973] is a global test of fit and can be a useful addition to more informative item-level metrics [@johansson_detecting_2025]. However, it is sensitive to large sample sizes and number of items, and produces false positives, see <https://pgmj.github.io/clrt.html> for a very small simulation study. To avoid the sample size issue, a bootstrap function has been implemented, where one can set the sample size manually. The function will then sample with replacement from the response data and conduct a separate LRT for each sample drawn. 

```{r}
RIbootLRT(df, iterations = 1000, samplesize = 400, cpu = 8)
```

If data fits the Rasch model well, the test will usually not be statistically significant.
...
### LRT
```{r}
RIbootLRT(df, iterations = 1000, samplesize = 500, cpu = 8)
```
...
There are still problematic residual correlations and several items show misfit, but we will end this sample analysis here and move on to show other functions.
