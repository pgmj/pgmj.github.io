...
The Conditional Likelihood Ratio Test [LRT, @andersen_goodness_1973] is a global test of fit and can be a useful addition to more informative item-level metrics [@johansson_detecting_2025]. However, it is sensitive to large sample sizes and may produce false positives, see <https://pgmj.github.io/clrt.html> for a very small simulation study. To avoid the sample size issue, a bootstrap function has been implemented, where one can set the sample size manually. The function will then sample with replacement from the response data and conduct a separate LRT for each sample drawn. 

```{r}
RIbootLRT(df, iterations = 1000, samplesize = 500, cpu = 8)
```

If data fits the Rasch model well, the test will not be statistically significant. 
