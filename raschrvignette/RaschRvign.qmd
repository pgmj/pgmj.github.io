...
Since we have a sample size over 500, ZSTD item fit values would be inflated if we used the whole sample [@hagell2016;@muller_item_2020]. To better estimate accurate ZSTD values, the `RIitemfitPCM()` function allows for multiple subsampling. While it has been recommended to use a sample size between 250 and 500 [@hagell2016], it is better to keep the sample size at 200-250 when possible  [@muller_item_2020]. If your data has few responses in some response categories for some items, you may need to try larger number of participants.
...
Conditional item fit using `iarm::out_infit()` has been implemented (since version 0.1.40.0, released on August 6th 2024) as the default method in `RIitemfitPCM()`, `RIitemfitPCM2()`, and `RIitemfitRM()`.
