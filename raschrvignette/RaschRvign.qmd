...
```{r}
RIitemfitPCM2(df, samplesize = 250, nsamples = 32, cpu = 8)
```

"Outfit" refers to item fit when person location is relatively far away from the item location, while "infit" provides estimates for when person and item locations are close together. MSQ should be close to 1, with lower and upper cutoffs set to 0.7 and 1.3 as default values, while ZSTD should be around 0, with default cutoffs set to +/- 1.96. Infit is usually more important. You can change the cutoff values by using options in the function, see `?RIitemfitPCM` for details. The cutoff values are debatable, and the curious reader should read Marianne Müller's paper on the topic [-@muller_item_2020]. During the [Scandinavian Applied Measurement Conference](https://www.hkr.se/en/collaboration/samc2024/) in June of 2024, [Karl Bang Christensen](https://publichealth.ku.dk/staff/?pure=en/persons/64551) presented very interesting work on simulation based cutoff values. There is a [Shiny app available for dichotomous data](https://shiny.sund.ku.dk/RMDitemfit/), but to my knowledge this work is not yet available for polytomous data.

A low item fit value (sometimes referred to as "overfitting" the Rasch model) indicates that responses are too predictable and provide little information. This is often the case for items that are very general/broad in scope in relation to the latent variable. A high item fit value (sometimes referred to as "underfitting" the Rasch model) can indicate several things, often multidimensionality or a question that is difficult to interpret. This could for instance be a question that asks about two things at the same time or is ambiguous for other reasons.

Since we have a sample size over 400, ZSTD item fit values would be inflated if we used the whole sample [@hagell2016;@muller_item_2020;smith_rasch_2008]. To better estimate accurate ZSTD values, the `RIitemfitPCM()` function allows for multiple subsampling. While it has been recommended to use a sample size between 250 and 500 [@hagell2016], it is better to keep the sample size at 200-250 when possible [@muller_item_2020], avoid over 400 [@smith_rasch_2008]. If your data has few responses in some response categories for some items, you may need to try larger number of participants.

We will set the sample size to 250 and run 32 subsamples. If you just want to test things out, I highly recommend lowering the number 32 to 4 to enable faster rendering while getting approximate values. Based on experience, 32 subsamples seems to result in stable values, but I have yet to systematically evaluate this.
...
Conditional item fit [@muller_item_2020] using `iarm::out_infit()` [@mueller_iarm_2022] has been implemented (since version 0.1.40.0, released on August 6th 2024) as the default method in `RIitemfitPCM()`, `RIitemfitPCM2()`, and `RIitemfitRM()`.

### Conditional item fit curves

The [`iarm`](https://cran.r-project.org/web/packages/iarm/index.html) package [@mueller_iarm_2022] provides several interesting functions for assessing item fit, DIF and other things. Some of these functions may be included in a future version of the `RISEkbmRasch` package. Below are conditional item characteristic curves (ICC's) using the estimated theta (factor score).
...
The `iarm` package also provides functions for assessing conditional item fit [@muller_item_2020]: *"To avoid bias observed item responses are compared to expected responses under the conditional distribution of responses given the total score."* (text from <https://search.r-project.org/CRAN/refmans/iarm/html/out_infit.html>)

Conditional item fit using `iarm::out_infit()` has been implemented (since version 0.1.40.0, released on August 6th 2024) as the default method in `RIitemfitPCM()`, `RIitemfitPCM2()`, and `RIitemfitRM()`. There is however more information available from the `iarm` output, if desired.

```{r}
r <- PCM(df) # run the partial credit model (not necessary to do again, but included here for clarity)
out_infit(r)
```

For correct p-values and standard errors using bootstrapping, one can use `boot_fit(r, B = 100)` instead of `out_infit(r)`.
...
Item fit looks generally quite good.
...
```{r}
RIitemfitPCM2(df2, 250, 32, cpu = 8)
...
`RIestThetasOLD()` can be used with a pre-specified item (threshold) location matrix. The choice of WL as default is due to the lower bias compared to ML estimation [@warm1989]. Similarly to `RIscoreSE()` you can (and will often need to) change the range of logit scores, using the option `theta_range` (default is `c(-4,4)`).
