...
Reliability (or measurement uncertainty) is often reported as a point estimate only, which implies constant reliability across the latent variables. This does not reflect the targeting aspect and the limitations of reliability at the outskirts of the measure. One way to try to better describe the varying reliability is by using the Test Information Function (TIF), which is curve. I have previously advocated for reporting the TIF curve, but have also found some unexpected results when using it. When I finally sat down to try to figure out the issues with TIF, I found a paper showing that it behaves quite erratically, especially with few items [@milanzi_reliability_2015]. While the `RItif()` function remains, I advice careful use of it. The new (since `easyRasch` 0.4.0) function `RIreliability()` will produce several point estimates of reliability. One of these is based on a recent preprint [@bignardi_general_2025] which describes a general reliability metric that includes uncertainty around the point estimate, called RMU, "Relative Measurement Uncertainty". Bignardi and colleagues use Bayesian estimation and posterior draws of each respondents latent scores (thetas/locations) to determine the mean and 95% highest density continuous interval (HDCI) of the distribution of correlations between draws [@bignardi_general_2025]. I have adapted code that the authors made available, and adapted it to simpler use by grace of the `TAM` package and its function to generate plausible values [@mislevy_randomization-based_1991;@wu_role_2005] instead of having to learn to specify Bayesian models. Preliminary investigations indicate similar results, see the example code in the documentation `?RIreliability` for an example.

In addition to the RMU, `RIreliability()` outputs the Person Separation Index (PSI), the Expected A Posteriori (EAP), and a table with SEM values from the WL estimation. I suggest reporting PSI, EAP and RMU in publications. Also, the default setting is to use 1000 plausible values, and I think it would be prudent to use something like 4000 for a publication. Maybe more.

To me, there are some outstanding issues with the reliability metrics produced by `RIreliability()`. For one, the TIF curve was based on item parameters, describing the test reliability. All point estimates are sample specific, describing the sample reliability. I think both are relevant, depending on what you are doing. For validating a measure, it would be most interesting to present the measures reliability. Also, it would be very neat to have a non-constant metric that describes the test reliability. And I still believe that a reliability curve would be optimal, rather than a point estimate with or without uncertainty interval. One way to look at this would be to use the estimated standard error of measurement from the Weighted Likelihood Estimation of person locations/scores, since these are varying across the latent continuum. A recent paper [@mcneish_reliability_2025], focusing on factor analysis, also arguing for the relevance of a conditional reliability
