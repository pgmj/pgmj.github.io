...
::: {.callout-note}
In the tabset-panel below, each tab will have some explanatory text. Remember to scroll back up and click on all tabs.
:::
...
```{r}
simfit <- RIgetfit(df, iterations = 1000, cpu = 8) # save simulation output into object `simfit` (arbitrary name)
...
Briefly stated, the simulation uses the properties of the current sample and items, and simulates n iterations of data that fit the Rasch model to get an empirical distribution of item fit that we can use for comparison with the observed data.

The simulation will take quite a bit of time to run if you have complex data/many participants, and/or choose to use many iterations. While insufficient testing has been done to make any strong recommendations, I think 1000 iterations is a decent starting point, and certainly no less than 500.

For reference, the simulation above, using 10 items with 5 response categories each and 1851 respondents, takes about 145 seconds (~2.5 minutes) to run on 8 cpu cores (Macbook Pro M1 Max).
...
```{r}
simfit3 <- RIgetfit(df, iterations = 1000, cpu = 8) # save simulation output into object `simfit` (arbitrary name)
...
```{r}
res_cor3 <- RIgetResidCor(df, iterations = 1000, cpu = 8)
RIresidcorr(df, cutoff = res_cor3$p99)
```
...
df %>% 
  ...
  filter(!rowname %in% misfits) %>% 
  dplyr::select(!rowname) %>% 
  RIitemfit(simcut = simfit3)
```

The same items are highlighted in red as when the misfitting persons were included in the analysis.
