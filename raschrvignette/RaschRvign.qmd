...
```{r}
simfit1 <- RIgetfit(df, iterations = 400, cpu = 8) # save simulation output to object `simfit1`
...
The simulation can take quite a bit of time to run if you have complex data/many items/many participants, and/or choose to use many iterations. Testing indicates that 200-400 iterations should be useful. Fewer than 200 seems to result in unstable values while more than 400 iterations does not improve detection rate, but may actually decrease it.
...
This is another useful function from the `iarm` package. It shows the expected and observed correlation between an item and a score based on the rest of the items [@kreiner_note_2011]. Similarly, but inverted, to item fit, a lower observed correlation value than expected indicates underfit, that the item may not belong to the dimension. A higher than expected observed value indicates an overfitting and possibly redundant item.
...
Based on rule of thumb, the first eigenvalue should be below 2.0 to support unidimensionality. However, this is seldom accurate and needs to be complemented with checking item fit and residual correlations. The target value should probably be closer to 1.5, based on my experience, but really I find this metric redundant and only keep it here for those coming from Winsteps who might be looking for it. Speaking of Winsteps, the "explained variance" will not be comparable to Winsteps corresponding metric, since this one only shows the results from the analysis of residuals and not the explained variance of the Rasch model itself.

### Residual correlations

Similarly to item fit, we need to run simulations to get a useful cutoff threshold value for when residual correlations amongst item pairs are larger than would be expected from items that fit the Rasch model [@christensen2017].
...
```{r}
simfit2 <- RIgetfit(df, iterations = 400, cpu = 8)
...
```{r}
simfit3 <- RIgetfit(df, iterations = 400, cpu = 8)
