...
I recently needed to do a power analysis for a longitudinal study, and since I always do my explorative/learning in a Quarto file it seemed reasonable to take a few minutes to share my experiences.
...
I made some changes in the example, regarding time points, clusters and icc's. Below is a test run for one combination of variable settings.
...
For this example, we will create a function that allows us to vary some of the input parameters: 

- the number of participants per sample
...
Next, we will define the parameter variations we want to look at.
...
And now the magic of `expand_grid()` to create a dataframe with all combinations of the three variables.
```{r}
# make a dataframe with all combinations
combinations <- expand_grid(sampleSizes, dropoutRates, effectSizes)
```

We'll make use of `pmap()` since we need to input more than 2 variables.
...
df.power %>%
  ...
  labs(
    ...
    caption = "Note. There are four clusters and five time points."
  ) +
  theme_minimal(
    base_family = "Lato",
    base_size = 14
