...
sim_results_4t <- foreach(i = 1:iterations) %dopar% {
  ...
  # center to 0
  input_params_c <- input_params - mean(input_params)
  # estimate thetas
  ...
  erm_params_c <- erm_params - mean(erm_params)
  ...
  tam_params_c <- tam_params - mean(tam_params)
  ...
  mirt_params <- coef(mirt_out, simplify = TRUE, IRTpars = TRUE)$items %>%
    ...
    set_names(c("T1","T2","T3","T4")) %>%
    as.matrix()
  mirt_params_c <- mirt_params - mean(mirt_params)
  ...
  pair_params_c <- pair_params - mean(pair_params)
