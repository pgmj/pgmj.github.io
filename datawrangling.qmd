...
This example assumes that you have previously created a DIF variable (vector) for gender with two groups (Female/Male).
...
```
The last part will make the tile plots show up side by side, labeled with the gender variable as it is coded in the dataset. If you have the gender variable as a factor (recommended), you will find it written out as a text string in the title of the tile plot.
