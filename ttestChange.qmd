...
```{r}
RIreliability(td, draws = 1000)
```

Separation Reliability: 0.8295
WLE_empirical = 0.854
WLE-RMU = 0.842 (95% HDCI [0.831, 0.853]) (1000 draws) using package mirt and 50 RMU iterations.
...
tt_results %>% 
  group_by(group) %>%
  count(result95) %>%
  mutate(Percent = n * 100 / sum(n) %>% round(2)) %>% 
  kbl_rise(tbl_width = 40) %>%
  row_spec(c(1,4),bold=TRUE)
...
Only using random intercepts, no random slopes.
...
m0 <- lmer(erm_thetas ~ time * tx + (1 | id),
           data = d_long)

DHARMa::simulateResiduals(m0, plot = T)
performance::check_outliers(m0)
```

Outliers, which is not uncommon since the distribution of estimated thetas typically leads to fatter tails. Let's compare gaussian with students t.


```{r}
library(glmmTMB)

m1 <- glmmTMB(erm_thetas ~ time * tx + (1 | id),
          data = d_long,
          family = "gaussian")

m2 <- glmmTMB(erm_thetas ~ time * tx + (1 | id),
          data = d_long,
          family = "t_family")

AIC(m1,m2)
glmmTMB::family_params(m2)
DHARMa::simulateResiduals(m2, plot = T)


comp <- avg_comparisons(m2, variables = list(time = c(1,3)), by = "tx") 
comp
hypotheses(comp, "b2 = b1")
